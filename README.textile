...
A simple (and scrappy) graph stress tester based on the Pilot graph operator, utilizing the Tinkerpop stack -- i.e. Blueprints, Gremlin, Pipes. You can use it to:
...
bc. $ ./GraphStressTester <testdefinitionfile.xml>

p. The test definition file is pretty straightforward XML (see sampletestdefinition.xml for a comprehensive example) like so:
...
bc. ------xxxxxxxxxxxxxxxx (tinkergraph_100vertices_linear0.2connxns--getneighbors) ------
